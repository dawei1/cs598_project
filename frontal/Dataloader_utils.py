'''
This module parses CSV files that contain metadata about each image in the
CheXpert dataset. The output is saved as pickle files that are then used by
by the Dataloader. This module ensures that we are only using images that are
frontal PA images.
'''
import Constants
    ...
    with open(csv_path) as csv_file:
        ...
        for row in dataset_info:
            ...
            if (not col_name_found):
                ...
                continue
            image_dic = {}
            image_dic['imagePath'] = Constants.DatasetRootDir+row[0].replace(Constants.dir_prefix, '')
            image_dic['sex'] = row[1]
            image_dic['age'] = row[2]
